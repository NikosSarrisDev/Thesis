...
const fieldDisplayNames = {
    ...
    given_name: "Years of Studies",
    nickname: "Student id",
    middle_name: "Address",
    family_name: "Phone Number",
};

const CheckalreadyCredential = ({ user }) => {
        ...
        if (canContinue) {
            ...
            swal({
                ...
                swal({
                    ...
                    // userManager.getUser()
                    // userManagerOlympus.storeUser()
                    setTimeout(() => window.open("http://localhost:3000/applicationForm", "_self"), 1000);
                });
            });
        } else {
            // User does not meet the criteria, show a warning and initiate logout
            swal({
                title: "Details Not Revealed, Invalid Corriculum",
                text: "Please select at least one detail to reveal and make sure you are from the new Curriculum.",
                icon: "warning",
                button: "Ok",
            }).then(() => {
                // Initiate logout
